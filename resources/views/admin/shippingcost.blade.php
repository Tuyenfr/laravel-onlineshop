                                    ...
                                    @foreach ($shippingcosts as $shippingcost)
                                        <tr>
                                            ...
                                            <td style="display: flex">
                                                <a href="{{ url('admin/editshippingcost', [$shippingcost->id]) }}"
                                                    class="btn btn-primary btn-xs">Edit</a>
                                                <form action="{{ url('admin/deleteshippingcost', [$shippingcost->id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="submit" class="btn btn-danger btn-xs"
                                                        style="margin-left: 5px" value="Delete">
                                                </form>
                                            </td>
                                    @endforeach
                                </tbody>
        ...
        <section class="content">
            <div class="row">
                <div class="col-md-12">
                    ...
                    @foreach ($rows as $row)
                        <form class="form-horizontal" action="{{ url('admin/updateshippingcost', [$row->id]) }}"
                            method="post">
                            @csrf
                            @method('PUT')
                            <div class="box box-info">
